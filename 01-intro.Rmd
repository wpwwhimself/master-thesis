...
- *Wielki Kryzys (Great Depression)* (1929-1932) -- największy kryzys gospodarczy w dziejach kapitalizmu, który swoim zasięgiem objął cały świat, z wykluczeniem ZSRR.
...
Próbując zapobiec utracie kruszca, podniesiono stopy procentowe, co w konsekwewncji znacząco wpłynęło na&nbsp;produkcję przemysłową wielu krajów [@greatDepressionWiki],
...
Bank centralny wkrótce zdecydował się na&nbsp;podwyższenie stóp procentowych, co zaowocowało brakiem możliwości spłaty długów przez zadłużonych, oraz spadkiem cen papierów wartościowych, będącym konsekwencją ich masowego wykupowania.
...
Na&nbsp;przestrzeni ostatnich lat światem wstrząsnęły również kolejne dramatyczne wydarzenia, mianowicie:

- __epidemia SARS-CoV-2__ w listopadzie 2019 r. -- pandemia wywołana chorobą _COVID-19_ (powodowana przez koronawirusa _SARS-CoV-2_),
gdzie pierwszy zidentyfikowany przypadek zachorowania na&nbsp;tę chorobę został wykryty w grudniu 2019 roku w Wuhan (Chiny).
...
Dodatkowo zarażeni wykazują oznaki osłabienia, nagłej utraty węchu i smaku oraz wielu innych dolegliwości.
...
Pandemia przyniosła znaczne skutki socjoekonomiczne, w tym też największą recesję od czasów *Great Depression* [@covidIMF].
...
Rosja w odpowiedzi zakazała swoim rezydentom przelewów walut obcych na&nbsp;konta zagraniczne, a także&nbsp;wymusiła płatności w rublach za surowce sprowadzane z Rosji, np. gaz ziemny.
...
Wyniki ich badań zostały opublikowane na&nbsp;łamach różnego rodzaju czasopism i publikacji
...
Szczególną zaletą modeli przełącznikowych jest możliwość uwzględnienia zmian we względnym poziomie zmienności -- parametru określającego intensywność wahań wartości szeregu czasowego.
Pozwala to wyróżnić okresy wysokiej i niskiej zmienności i oddzielić momenty wysokich wstrząsów od czasów relatywnie spokojnego rynku.
...
-- ze względu na&nbsp;różnorakie limitacje (czasowe i&nbsp;komputacyjne) ograniczony został zakres badania,
co może ostatecznie nie dawać pełnego obrazu sytuacji panującej na&nbsp;warszawskiej giełdzie.
Dokładna analiza tego i innych wad przeprowadzonej analizy została dokładniej omówiona w rozdziale 3.
Mimo to obrany horyzont czasowy, wraz z przyjętym wolumenem badanych spółek i zakresem weryfikowanych modeli ekonometrycznych,
pozwalają przypuszczać, że&nbsp;wyniki niniejszego badania są wiarygodne i wnioski z niego wyciągnięte mają swoje ugruntowanie w&nbsp;rzeczywistości.
...
**Indeksem giełdowym** nazywa się wskaźnik, który służy do monitorowania cen papierów wartościowych spółek notowanych na&nbsp;giełdzie.
Jest to wskaźnik umożliwiający śledzenie trendów panujących na&nbsp;rynkach finansowych, bez konieczności monitorowania dużej liczby cen akcji poszczególnych spółek.
...
Na&nbsp;giełdzie warszawskiej funkcjonuje kilka indeksów, a największymi z nich są [@indeksBI;@wigGpwBenchmark]:
...
Dane w szeregu czasowym mogą reprezentować pomiary dzienne, miesięczne, roczne lub dowolny inny interwał czasowy.
...
Dla szeregów czasowych reprezentujących dane finansowe częstą praktyką jest konstruowanie szeregu **logarytmicznych stóp zwrotu** z inwestycji z wykorzystaniem wzoru:
...
- modelowanie zmienności -- analiza stóp zwrotu jest przydatna na&nbsp;potrzeby modeli poruszanych w tym badaniu, które są skoncentrowane na&nbsp;analizie zmienności zjawiska zamiast jego rzeczywistych wartości.
...
Ma to swoje zastosowanie w klasie modeli **ARMA** (_autoregressive - moving average_), którego budowę przedstawia wzór:
...
w którym wariancja warunkowa modelu jest opisywana przez kwadraty reszt z modelu:
...
- rozkład normalny -- średnia $\mu = 0$, wariancja $\sigma^2$,
...
Określa się wiele rodzajów błędów, ale do najczęściej wyróżnianych w badaniach zalicza się te opisane poniżej.
...
Oczywiście do każdych danych istnieje możliwość dopasowania kilku różnych modeli, wobec czego zachodzi potrzeba porównania ich jakości estymacji.
...
Powoduje to również nadmiernie uproszczenia w edukowaniu o istnieniu tychże kryteriów i prowadzi do zbyt zlaicyzowanego przeświadczenia o bezwzględnych korzyściach pewnych kryteriów nad pozostałymi.
...
a spośród dostępnych testów wykorzystane zostały testy opisane poniżej.
Do każdego z&nbsp;nich podano wzór oraz rozkład statystyki testowej przy prawdziwości hipotezy zerowej.
Informacje te nie są kompleksowe, a po więcej szczegółów czytelnika odsyła się do opracowań źródłowych.

- Test normalności rozkładu zmiennej losowej
-- test Shapiro-Wilka [@shapiroTest], którego $H_0$ zakłada, że dane cechują się rozkładem normalnym.
...
- Test autokorelacji rzędu $l$ w obrębie szeregu czasowego
-- test Ljung-Boxa [@boxTest], którego $H_0$ zakłada brak autokorelacji w szeregu czasowym.
Statystyka testowa dana jest wzorem:
\begin{equation} (\#eq:ljung)
...
- Testy na&nbsp;występowanie efektu ARCH w resztach z modelu liniowego dla szeregu czasowego
  - test McLeoda-Li [@mcLeodLiTest], zakładający brak efektu ARCH przy $H_0$ poprzez brak autokorelacji w szeregu kwadratów reszt z modelu liniowego.
    Test ten jest tożsamy z testem Ljung-Boxa (wzór \@ref(eq:ljung)), z jedyną różnicą w danych wykorzystanych do&nbsp;obliczenia statystyki testowej;
...
Najczęstszymi wyborami badaczy zajmującymi się tymi zagadnieniami są programy oraz języki wspierające budowę skomplikowanych równań,
...
Do przeprowadzenia wszelkich obliczeń i symulacji w niniejszej pracy wykorzystano środowisko **R** [@R].
Jest to otwartoźródłowy język programowania i środowisko do analizy danych oraz tworzenia grafik,
powszechnie używane w statystyce, analizie danych, uczeniu maszynowym oraz badaniach operacyjnych.
Charakteryzuje się bogatym zestawem bibliotek (pakietów), które umożliwiają wykonywanie różnorodnych analiz i wizualizacji danych.

Na&nbsp;potrzeby tego badania użyte zostały pakiety zaprojektowane do pracy z szeregami czasowymi oraz&nbsp;modelami ekonometrycznymi.
Do pracy z modelami GARCH zastosowano dwie z najpopularniejszych bibliotek zaprojektowanych do tego celu:

- `rugarch` [@packageRugarch] -- pakiet, którego celem jest zapewnienie elastycznego i bogatego środowiska do&nbsp;modelowania i testowania modeli GARCH.
...
Zastosowany został do estymacji modeli z zastosowaniem zewnętrznych regresorów, oferując subiektywnie najbardziej przystępny sposób przekazania tych danych do programu (interfejs) spośród innych oferowanych rozwiązań.
- `MSGARCH` [@packageMSGARCHDocs] -- implementacja kompleksowego zestawu funkcjonalności na&nbsp;potrzeby modelu przełącznikowego Markowa z zastosowaniem modeli GARCH,
...
W skład środowiska OxMetrics wchodzą pakiety pozwalające m.in. na&nbsp;[@packageOx]:

- konstruowanie modeli ekonometrycznych (pakiet `PcGive`),
- estymację i prognozowanie modeli klasy GARCH (pakiet `G@RCH`),
- statystyczną/ekonometryczną analizę szeregów czasowych, a także ich modelowanie i predykcję (pakiet `STAMP`).

Innym programem pozwalającym na&nbsp;kompleksową analizę szeregów czasowych jest pakiet **Time Series Modelling** [@packageTSM].
...
Skutkiem wysokiej popularności języka **Python**, znanego z posiadania szerokiego zestawu bibliotek rozszerzających jego działanie oraz przyjaznej dla początkujących programistów składni,
jest rozwój pakietów do tego języka skoncentrowanych na&nbsp;analizie modeli szeregów czasowych.

- Na&nbsp;potrzeby analizy modeli ARIMA jest to m.in. pakiet `pmdarima` [@packagePythonPmdarima].
...
wykorzystuje znany użytkownikom model programistyczny pozwalający na&nbsp;przejrzystą i kompleksową analizę,

- Do budowania modeli GARCH popularną biblioteką jest `arch` [@packagePythonArch].
Jest ona zaprojektowana do&nbsp;modelowania zmienności warunkowej za pomocą różnych modeli klasy GARCH (GARCH, GJR, TARCH, EGARCH)
a także na&nbsp;prognozowanie zmienności warunkowej oraz analizę wartości zagrożonej,

- Na&nbsp;analizę modeli przełącznikowych Markowa natomiast pozwala pakiet `statsmodels` [@packagePythonStatsmodels].
...
Implementuje także budowę modeli przełącznikowych, takich jak model Markowa.

### Julia

Ostatnim z języków programowania, o jakich warto wspomnieć na&nbsp;łamach niniejszej pracy, jest **Julia** [@packageJulia].
...
Zaskarbia sobie rosnące zainteresowanie w różnych dziedzinach nauki, np. biologii, fizyce czy astronomii.

Z uwagi na&nbsp;dojrzewającą bazę bibliotek tego języka,
możliwości pracy w zakresie modeli GARCH oraz przełącznikowych są stosunkowo ograniczone.
...
- `ARCHModels.jl` [@packageJuliaARCHModels] --
...
- `MarSwitching.jl` [@packageJuliaMarSwitching] --
