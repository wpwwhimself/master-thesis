...
co daje `r length(tables)` szeregów czasowych, z których każdy zawiera `r length(na.omit(returns_split[[1]]))` obserwacje.
...
Na&nbsp;Rysunku \@ref(fig:returns-plot) przedstawione są wykresy logarytmicznych stóp zwrotu dla spółek wykorzystanych w badaniu.
...
Dla pozostałych szeregów czasowych model białego szumu również zgadza się z&nbsp;brakiem zaobserwowanej autokorelacji.
...
Może to sugerować, że&nbsp;modele wyłonione pod kątem kryterium Akaike są równie dobrze dopasowane do danych, co te wyłonione przez kryterium Schwarza.
...
Test McLeoda-Li, którego wyniki zostały przedstawione na&nbsp;korelogramach na&nbsp;Rysunku \@ref(fig:arma-arch-acf),
...
Wartości parametrów we wszystkich modelach (występujące we wzorach w rozdziale 1.) podkreślają różnice między każdym z reżimów.
Można zauważyć, że reżim drugi cechuje się wyższą średnią zmiennością (parametr $\omega$),
a niemal we wszystkich modelach współzależność między obserwowanymi innowacjami $\beta$ jest wyraźnie silniejsza od tej związanej z autokorelacją reszt modelu liniowego $\alpha$.
Uzasadnia to wykorzystanie modeli GARCH(1,1), gdzie zastosowanie niskich opóźnień wystarcza do dobrego objaśnienia trajektorii zmienności.
Dodatkowe parametry, takie jak $\gamma$ w modelach EGARCH i GJR, a także parametry związane z rozkładem innowacji $\nu$ i $\xi$, wykazują wyraźnie większy udział w reżimie drugim.
Reżim ten może być więc interpretowany we wszystkich przypadkach jako model starający się wyjaśnić większy chaos związany z kryzysami.
Prawdziwość tej tezy łatwo zauważyć na Rysunku \@ref(fig:markov-plots), gdzie wyróżniony reżim drugi pokrywa się z analizowanymi przedziałami czasowymi związanymi z okresem pandemii oraz konfliktem na&nbsp;wschodzie.
...
za zmienne objaśniające przyjęte zostały 4 szeregi czasowe.
Pierwszym z nich jest zmienna zero-jedynkowa, wyróżniająca istotne okresy czasu, związane z tymi wydarzeniami na&nbsp;scenie ekonomicznej Polski.
...
Pozostałymi z przyjętych zmiennych są szeregi logarytmicznych stóp zwrotu największych indeksów giełdowych: WIG, S&P oraz DAX.
Stoi za&nbsp;tym przekonanie, że nawet jeśli dana spółka nie&nbsp;jest składową danego indeksu,
...
Zaobserwowane wahania zmienności tych indeksów będą wówczas odzwierciedlały wpływ czynników zewnętrznych dla każdej z analizowanych firm.
...
group_names <- rep(1, length(tables))
names(group_names) <- tables_full_names
...
table <- map(
  ...
  select(-starts_with(c("ar", "ma")), -series)
...
table %>%
  ...
  add_header_above(c(
    " " = 1,
    "Parametry modelu" = length(colnames(table)[colnames(table) %in% c(
      "omega", "alpha1", "beta1", "gamma1", "vxreg1", "vxreg2", "vxreg3", "vxreg4"
    )]),
    "Par. rozkł. inn." = length(colnames(table)[colnames(table) %in% c(
      "skew", "shape"
    )])
...
Można zauważyć, że każdy z modeli uwzględnia asymetrię modelu, ponieważ często wyłoniony został model EGARCH.
...
Na podstawie tych parametrów widać względnie niski wpływ czynników zewnętrznych na wartość akcji firm.
Może być to spowodowane liczbą wykorzystanych szeregów, powodując rozproszenie udziału każdego z nich.
Spośród tych czynników, odgórne wyróżnienie okresów kryzysu $\lambda_1$ osiąga wyższe wartości parametrów w porównaniu z tymi związanymi z indeksami giełdowymi WIG, S&P oraz DAX (kolejno $\lambda_2$, $\lambda_3$ i $\lambda_4$).
W tym zestawieniu istnieją 2 spółki, gdzie czynniki zewnętrzne mają bardzo znikomy wpływ na kształtowanie się trajektorii szeregów czasowych spółki.
W kontraście do&nbsp;tego zjawiska, parametry rozkładu innowacji dla tych firm osiągają wyższe wartości w odróżnieniu od pozostałych modeli.
Dopasowane modele, podobnie jak w przypadku modeli przełącznikowych, charakteryzują się dużym udziałem autoregresji innowacji $\beta$.
...
Przedstawione pomiary błędu dopasowania nie przynoszą optymistycznych rezultatów.
Pierwszym faktem, na jaki należy zwrócić uwagę, są problemy w obliczeniu zmienności zrealizowanej dla niektórych z badanych szeregów czasowych.
Jako przykład wyróżnia się tu spółka _PKO BP_, dla której wskazania wyestymowanego odchylenia standardowego nie przekraczają poziomu 10 zł (patrz Rysunek \@ref(fig:prediction-volatilities)).
Wraz z danymi spółki _Synthaverse_, oba zestawy danych giełdowych cechują&nbsp;się stosunkowo niskimi wartościami instrumentu -- nie przekraczającymi kwoty 50 zł (Rysunek \@ref(fig:stock-prices-plot)).
Biorąc pod uwagę, że różnice w cenach akcji są głównymi elementami równania estymatora zmienności zrealizowanej,
przynosi to niewielkie wahania śróddzienne wartości instrumentów, a&nbsp;w&nbsp;konsekwencji niskie wskazania estymatora.
Z tego też powodu dla owych dwóch spółek wskazania błędu MAPE wyniosły powyżej 150%.
Wyjątkowo niskie wyniki zmienności zrealizowanej prowadzą do niebotycznych rozbieżności w stosunku do modelowanych wartości.

Dla pozostałych spółek błędy dopasowania plasują się od 40 do 100%, na podstawie wskazań MAPE.
Wyniki te oznaczają duże pole do poprawy modeli, z racji poziomu MAPE powyżej 50%.
Jedyną spółką o zadowalającym stopniu dopasowania modelu do danych jest _PKN Orlen_, której to modele wykazują wyjątkowo dobre estymacje zmienności warunkowej oraz prognoz.
