...
Aby zastosować model przełącznikowy do tego typu danych, konieczne jest dodatkowe przetworzenie danych [@packageMSGARCHDocs, str. 2].
...
Najlepsze z nich zostały wyłonione na podstawie kryterium informacyjnego Akaike (konsekwentnie z wyborem poczynionym w poprzednim rozdziale),
a także prawdopodobieństw utrzymania się i zmiany reżimu dla każdego z modeli.
Do wyłonienia optymalnego modelu posłużono się metodą TOPSIS.
Można zauważyć, że dla reżimu pierwszego (niska wariancja warunkowa) często najlepsze wyniki wykazuje standardowy model GARCH (`sGARCH`),
natomiast w przypadku drugiego reżimu, gdzie wahania stóp zwrotu są większe, częściej wybierany jest model EGARCH, uwzględniający asymetrię.
Badanie wykazało też brak konsensusu dotyczącego przyjętego rozkładu innowacji -- każdy z testowanych rozkładów pojawia się minimum dwukrotnie na liście najlepiej dopasowanych modeli.

```{r markov-re-fit}
markov_chosen_ids <-
  best %>%
  map(
    ~ markov_models_for_testing_indices[.x,] %>%
      as.numeric()
  )
    ...
    function(table_name) {
      ...
      CreateSpec(
        variance.spec = list(model = c(
          models_for_testing[markov_chosen_ids[[table_name]][1],]$volatility,
          models_for_testing[markov_chosen_ids[[table_name]][2],]$volatility
        )),
        distribution.spec = list(distribution = c(
          models_for_testing[markov_chosen_ids[[table_name]][1],]$distribution,
          models_for_testing[markov_chosen_ids[[table_name]][2],]$distribution
        )),
...
Tak wybrane modele zostały sprawdzone pod kątem istotności ich parametrów: parametry nieistotnie różne od zera zostały z góry ustalone jako równe 0.
Proces przeprowadzono dla parametrów, dla których można taką operację wykonać. \AAQ{Jak to jest z wymagalnością tych parametrów? Patrz} [@packageMSGARCH, str. 8]
Ma to na celu optymalizację modeli i poprawę wyników prognoz wykonywanych na dalszych etapach badania.
...
Ta analiza jednak przyniosła obiecujące rezultaty, mówiące o istotnie niezerowych wartościach wszystkich parametrów dla każdego ze sprawdzanych modeli
Wobec tego ostateczne wartości wyestymowanych parametrów wybranych modeli przedstawia Tabela \@ref(tab:markov-post-optimizing),
...
\AAA{mam spore problemy znaleźć modele, które dostatecznie dobrze trzymają reżimy}
...
map(
  ...
  relocate(nu_1, xi_1, .before = alpha0_2) %>%
  c_kable(
...
```{r markov-plots, fig.cap="Dopasowanie modelu Markowa do danych WIG 20", fig.height=8, fig.width=6.5}
map(
  tables,
  function(table_name) {
    get(table_name) %>%
...
W celu dopasowania modelu GARCHX koniecznym jest przygotowanie dodatkowych zmiennych, mających na celu objaśnienie wpływu czynników zewnętrznych na zmienność wariancji warunkowej.
Ponieważ badanie rozważa wpływ dwóch konkretnych wydarzeń na polską gospodarkę, za zmienną objaśniającą została przyjęta zmienna indykatorowa, wyróżniająca istotne okresy z nimi związane.
Wielkość interwałów czasowych została określona w oparciu o Rysunek \@ref(fig:returns-plot).
Wyróżnionymi okresami są:

- 4.03.2020-1.01.2021 r. -- wybuch pandemii koronawirusa w Polsce,
- 24.02-1.06.2022 r. -- początek inwazji rosyjskiej na Ukrainie.

Podobnie jak w poprzednim podrozdziale, dla każdego z badanych szeregów czasowych dopasowano model GARCH z uwzględnieniem dodatkowych zmiennych
...
seq_along(stats) %>%
  ...
  bind_rows() %>%
  c_kable(
    "Najlepsze wartości kryteriów informacyjnych dla modeli GARCHX
    dopasowanych do stóp zwrotu badanych spółek",
    col.names = c(
      "Szereg czasowy",
      "Model",
...
Tabela \@ref(tab:garchx-models) przedstawia wyniki dopasowania modeli do danych spółek.
