...
Koncentruje się głównie na projektach mieszkaniowych w największych miastach Polski.
...
W ciągu 16 lat działalności firma oddała do użytku ponad 17 tys. lokali, a przykładem jej projektów w Poznaniu jest _Malta Point_ przy ul. Brneńskiej [@develiaWiki;@develiaSelf].
...
Dodatkowo, jako że mowa o krajowych liderach w swoich branżach, wyniki finansowe objętych badaniem firm są dobrym wyznacznikiem kondycji całego polskiego rynku.
Celem zróżnicowania badania, procedura badawcza zostanie zastosowana również dla **indeksu giełdowego WIG20**.
...
Punktem wyjścia do prowadzenia badań będzie utworzenie szeregu czasowego cen zamknięcia akcji.
...
- 11 bit studios -- wyraźna autokorelacja w opóźnieniach 1., 2., 5., 7. i 8.,
- PKN Orlen -- istotna autokorelacja w opóźnieniu 4.,
- PKO BP -- autokorelacja w opóźnieniach 1. i 5..

Na tej podstawie podczas testów dopasowania zbadano dopasowanie modeli ARMA do rzędów opóźnień równych `r arma_max_lag`.
...
Mimo to przeprowadzenie testów Ljunga-Boxa na występowanie autokorelacji w szeregach czasowych reszt z modelu dla tych wybranych na podstawie BIC wykazało fakt,
...
- McLeoda-Li (test Ljunga-Boxa dla kwadratów reszt z modelu) [@mcLeodLiTest],
- Engle'a [@engleTest],
- KPSS [@KPSSTest].
...
map(
  ...
  add_footnote(
    "Dla testów KPSS wartość 0.1 w kolumnie p-wartość oznacza, że rzeczywista p-wartość jest wyższa od wskazanej.",
    notation = "symbol"
  )
```

Test McLeoda-Li, który został przedstawiony na korelogramach na Rysunku \@ref(fig:arma-arch-acf), wskazuje na istotną autokorelację w szeregach kwadratów reszt z modelu,
...
Aby zastosować model przełącznikowy do tego typu danych, konieczne jest dodatkowe ich przetworzenie [@packageMSGARCHDocs, str. 2].
...
Można zauważyć, że optymalne wyniki wykazuje model EGARCH (`eGARCH`), uwzględniający asymetrię wartości szeregu czasowego.
...
```{r markov-plots, fig.cap="Prawdopodobieństwa obecności szeregów czasowych w odpowiednich reżimach dla modelu Markowa", fig.height=8, fig.width=6.5}
map(
  tables,
  function(table_name) {
    get(table_name) %>%
...
Ponieważ badanie rozważa wpływ dwóch konkretnych wydarzeń na arenie międzynarodowej na polską gospodarkę, za zmienną objaśniającą została przyjęta zmienna indykatorowa,
wyróżniająca istotne związane z tymi, a także innymi wydarzeniami na scenie ekonomicznej Polski.
...
garchx_externals <-
  ...
  mutate(
    is_crisis = Reduce(
      `|`,
      map(periods, ~ Data %within% interval(dmy(.x[1]), dmy(.x[2])))
    )
  ) %>%
  pull(is_crisis) %>%
  as.numeric()
...
# rozpisz
cat("\\begin{itemize}\n\\tightlist\n")
walk(periods, ~ cat(paste("\\item", .x[1], "-", .x[2], "r.:", .x[3], "\n")))
cat("\\end{itemize}")
```
...
map(
  ...
  select(-ar1, -ar2, -ma1, -ma2) %>%
  c_kable(
    "Parametry modeli GARCHX dopasowanych do stóp zwrotu badanych spółek",
    col.names = c(
      "Szer. czas.",
      "Model",
      "$\\omega$",
...
Tabela \@ref(tab:garchx-models) przedstawia modele GARCHX dopasowane do danych badanych spółek, wybrane na podstawie kryterium informacyjnego Akaike.
...
pred_vol <-
  list(
    ...
    garchx = garchx %>%
      map(~ .x %>%
        ugarchforecast(n_ahead = n_ahead) %>%
        sigma() %>%
...
a także prognozy, jakie zostały wykonane na `r n_ahead` dni naprzód.
