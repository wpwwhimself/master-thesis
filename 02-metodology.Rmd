...
Proces przeprowadzono dla parametrów, dla których można taką operację wykonać, tj. wszystkich poza $\alpha_1$ w modelu GARCH, z uwagi na konieczność jego wystąpienia.
Ma to na celu optymalizację modeli i poprawę wyników prognoz wykonywanych na dalszych etapach badania.

```{r markov-optimizing}
markov <- c_read_rds("markov")
...
map(
  ...
  function(table_name) {
    markov[[table_name]] %>%
      ...
      mutate(series = tables_full_names[table_name])
  }
) %>%
  bind_rows() %>%
  pivot_wider(
...
Jedynym modelem, który potrzebowałby takiej operacji okazał się być model dla szeregu czasowego stóp zwrotu indeksu WIG20,
gdzie wyestymowane wartości parametrów $\omega$ dla obu reżimów były nieistotnie różne od zera.
Jednakże po zablokowaniu ich wartości do 0 i ponownym procesie dopasowania modelu, wzrosły wartości jego kryteriów informacyjnych.
\AAQ{lepsze gorsze kryteria czy nieistotne parametry w modelu? Na razie zakładam, że to drugie}
Wobec tego ostateczne wartości wyestymowanych parametrów wybranych modeli przedstawia Tabela \@ref(tab:markov-optimizing).
