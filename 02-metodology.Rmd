...
W świetle tych wniosków w dalszej części badania modelem średniej warunkowej dopasowanym do badanych szeregów czasowych będzie **model białego szumu**,
z uwagi na jego mniej skomplikowaną budowę, co ma uprościć dalsze obliczenia.
...
Należą do nich testy <!-- - Ljunga-Boxa dla reszt z modelu, --> Engle'a oraz KPSS.
Do ich obliczenia wykorzystane zostaną pakiedy `rugarch` [@packageRugarch] oraz `tseries` [@packageTseries].
Wyniki tych testów ilustruje <!-- Rysunek \@ref(fig:arma-arch-acf) oraz --> Tabela \@ref(tab:arma-arch-tests).

```{r arma-arch-acf, fig.height=8, fig.cap="Korelogramy ilustrujące testy Ljunga-Boxa dla reszt z modelu średniej warunkowej", eval = F}
map(arma, residuals) %>%
  custom_acf_plot()
```
...
Wyniki testów Engle'a oraz KPSS sprzyjają dalszym rozważaniom;
...
models <- custom_read_rds("markov_wig20")
